$(function(){
	...
	function back() {
		location.href = "http://localhost:8080/crm/system/userList"
	}
	...
	$('textarea, input, select').blur(function(){
		...
		if(e.attr("data-validate")){
			...
			if($checkstate){
				e.closest('.field').removeClass("check-error");
				e.parent().find(".input-help").remove();
				e.closest('.field').addClass("check-success");
			}else{
				e.closest('.field').removeClass("check-success");
				e.closest('.field').addClass("check-error");
				e.closest('.field').append('<div class="input-help" style="color:red;"><ul>'+$checktext+'</ul></div>');
			}
		}
	});
